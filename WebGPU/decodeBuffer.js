...
function decodeBuffer(H, W, C) {
    const S = Math.ceil(C / 4);
    const WORKGROUP_SIZE = 16;
    const SRC_BUFFER_SIZE =  S * H * W * 4 * 4;
    const DST_BUFFER_SIZE =  C * H * W * 4;
    ...
    const simulationShaderModule = device.createShaderModule({
        ...
        fn computeMain(@builtin(workgroup_id) workgroup_id : vec3<u32>,
            ...
            if (outputIndex < ${DST_BUFFER_SIZE / 4}) {
                let c = outputIndex % ${C};
                let w = outputIndex / ${C} % ${W};
                let h = outputIndex / ${C} / ${W};
                let s = c / 4;
                let inputIndex = c % 4 + w * 4 + h * ${W} * 4 + s * ${H} * ${W} * 4;
                cellStateOut[outputIndex] = cellStateIn[local_invocation_id.x];
            }
        }
        `
    });
    ...
    computePass.dispatchWorkgroups(Math.ceil(C * H * W / WORKGROUP_SIZE), 1, 1);
    computePass.end();
